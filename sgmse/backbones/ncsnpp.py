...
class NCSNpp(nn.Module):
    ...
    def add_argparse_args(parser):
        parser.add_argument("--num_res_blocks",   type=int, default=1, help="how many res net blocks")
        parser.add_argument("--nf",   type=int, default=96, help="number of channels in res net blocs")
        parser.add_argument("--ch_mult", nargs="+",  type=int, default=[1, 2, 2, 2], help="Channel dimension multiplier per res net block. The length of the array corresponds to the number of resolutions.")        
        return parser
        ...
        **unused_kwargs
        ...
        for i_level in range(num_resolutions):
            ...
            for i_block in range(num_res_blocks):
                ...
                modules.append(ResnetBlock(in_ch=in_ch, out_ch=out_ch, frame_factor=frame_factor))
                in_ch = out_ch

                if all_resolutions[i_level] in attn_resolutions:
                    modules.append(AttnBlock(channels=in_ch))
                hs_c.append(in_ch)
            frame_factor = int(frame_factor*2)
            
            if i_level != num_resolutions - 1:
                if resblock_type == 'ddpm':
                    modules.append(Downsample(in_ch=in_ch))
                else:
                    modules.append(ResnetBlock(down=True, in_ch=in_ch, frame_factor=frame_factor))
        ...
        in_ch = hs_c[-1]
        frame_factor = int(frame_factor//2)
        modules.append(ResnetBlock(in_ch=in_ch, frame_factor=frame_factor))
        modules.append(AttnBlock(channels=in_ch))
        modules.append(ResnetBlock(in_ch=in_ch, frame_factor=frame_factor))
        ...
        for i_level in reversed(range(num_resolutions)):
            for i_block in range(num_res_blocks + 1):  # +1 blocks in upsampling because of skip connection from combiner (after downsampling)
                out_ch = nf * ch_mult[i_level]
                modules.append(ResnetBlock(in_ch=in_ch + hs_c.pop(), out_ch=out_ch, frame_factor=frame_factor))
                in_ch = out_ch
            ...
            if i_level != 0:
                if resblock_type == 'ddpm':
                    ...
                    modules.append(ResnetBlock(in_ch=in_ch, up=True, frame_factor=frame_factor))
    ...
    def forward(self, x, time_cond, y, scale_divide):
        # timestep/noise_level embedding; only for continuous training
        modules = self.all_modules
        m_idx = 0
        
        assert len(time_cond.shape) == 2
        assert len(x.shape) == 4
        assert len(y.shape) == 4
        assert len(scale_divide.shape) == 4
        ...
        if self.embedding_type == 'fourier':
            ...
            temb = modules[m_idx](torch.log(used_sigmas + 1e-2))
            m_idx += 1
