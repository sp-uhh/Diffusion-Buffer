    ...
    def pc_sampler():
        ...
        with torch.no_grad():
            ...
            for i in range(sde.N):
                ...
                xt, xt_mean = predictor.update_fn(xt, vec_t, y, conditioning=conditioning)
...
def get_cached_pc_sampler(
    ...
    def pc_sampler(noise_cache={}):
        """The PC sampler function."""
        with torch.no_grad():
            timesteps = torch.linspace(sde.T, eps, sde.N, device=y.device)
            # init noises
            if len(noise_cache) <= 1:
                noise_cache['prior'] = None
                noise_cache.update({f"corrector{i:03}": None for i in range(len(timesteps))})
                noise_cache.update({f"predictor{i:03}": None for i in range(len(timesteps))})

            xt, noise_cache = sde.prior_sampling_cached(y.shape, y, noise_cache)
            xt.to(y.device)
            for i in range(sde.N):
                t = timesteps[i]
                vec_t = torch.ones(y.shape[0], device=y.device) * t
                xt, xt_mean = corrector.update_fn(xt, vec_t, y, conditioning=conditioning)
                xt, xt_mean, predictor_step_noise_cache = predictor.update_fn(xt, vec_t, noise_cache[f'predictor{i:03}'], y, 
                                                  conditioning=conditioning)
                noise_cache[f'predictor{i:03}'] = predictor_step_noise_cache

            x_result = xt_mean if denoise else xt
            ns = sde.N * (corrector.n_steps + 1)
            return x_result, ns, noise_cache
